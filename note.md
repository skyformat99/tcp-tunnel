...
- 连接请求：`uv_connect_t`，当连接成功或失败，则调用我们传递给它的回调。请求对象有一个 handle 字段，存储与之关联的 handler 对象。回调函数的签名：`void (*uv_connect_cb)(uv_connect_t* req, int status)`。
- 写入请求：`uv_write_t`，当写入成功或失败，则调用我们的传递给它的回调。写入请求可能会排队，回调调用之后才能重用 write_req 对象。写入对象也有一个 handler 字段，指向执行此写入请求的 handler 对象。回调函数的签名：`void (*uv_write_cb)(uv_write_t* req, int status)`。
- 关闭请求：`uv_shutdown_t`，关闭请求的工作原理其实是这样的，因为 stream_t 对象有一个写入队列，当这个队列上的所有写入请求都执行完毕后，libuv 会发送 FIN 给对端，然后调用我们的回调。handle 字段同上。tcp-tunnel 项目上用不到 uv_shutdown()、uv_shutdown_t，因为我们同一时间只有一个 write_req 在进行。回调函数的签名：`void (*uv_shutdown_cb)(uv_shutdown_t* req, int status)`。
